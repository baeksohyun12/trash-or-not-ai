...
3. [시스템 전체 구조도](#시스템-전체-구조도)
4. [핵심 백엔드 아키텍처](#핵심-백엔드-아키텍처)
5. [핵심 프론트엔드 아키텍처](#핵심-프론트엔드-아키텍처)
6. [프로젝트 구조(Frontend) 및 설치 및 실행 방법](#프로젝트-구조frontend)
7. [프로젝트 구조(Backend) 및 설치 및 실행 방법](#프로젝트-구조-backend)
8. [환경정보](#환경-정보)
...
<img width="1276" height="607" alt="image" src="https://github.com/user-attachments/assets/f8593de4-309c-4e95-b0ab-a058327f874a" />

#### 3-1. AI 분리수거 서비스
1. 사용자가 사진/파일을 업로드합니다.  
2. 업로드된 파일을 검증한 후 AI 백엔드로 전송합니다.  
3. AI 모델이 이미지를 분석하여 쓰레기 종류를 분류합니다.  
4. 분석 결과를 사용자에게 화면으로 제공합니다.  
5. 사용자는 다시 분석하거나 다른 사진을 업로드할 수 있습니다.

#### 3-2. 미니게임
1. 사용자가 미니게임을 실행합니다.  
2. 게임 종료 후 결과 점수를 `localStorage`에 저장합니다.  
3. 다시 하기 또는 뒤로 가기를 선택할 수 있습니다.

#### 3-3. 클린하우스 위치 안내
1. 사용자의 현재 위치를 기반으로 지도를 초기화합니다.  
2. 주변 클린하우스 위치를 마커로 표시합니다.  
3. 거리 계산 후 가까운 클린하우스 정보를 제공합니다.

#### 시스템 구성
- **Frontend**: 이미지 업로드, 결과 화면 표시  
- **Backend**: 이미지 전달 및 AI 요청 처리  
- **AI Model**: 쓰레기 이미지 분류  
- **Kakao Map API**: 클린하우스 위치 표시
