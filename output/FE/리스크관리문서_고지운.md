...
> **담당 파트**: 사진 업로드 화면, 로딩 화면, AI 분석 기반 결과 화면, 미니 게임 구현<br>
> **작성 일자**: 2025.12.20 
...
```javascript
    // 1. 카메라 설정: 사람 눈과 유사한 원근감 적용
    const camera = new THREE.PerspectiveCamera(75, window.innerWidth / window.innerHeight, 0.1, 1000);
    camera.position.z = 2; // 카메라 위치 조정

    // 2. 렌더러 설정: 배경을 투명하게(alpha: true) 처리
    ...
    // 3. 반응형 처리: 화면 크기가 변할 때마다 카메라 비율 재계산
...
function initThreeJS() {
    ...
    // 1. 씬, 카메라, 렌더러 설정 (배경 투명화 alpha: true 적용)
